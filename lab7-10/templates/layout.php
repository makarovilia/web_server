<!DOCTYPE html>
<html lang="ru">
<head>

    <meta charset="UTF-8">

    <title>
        <?= $title ?? 'Мой блог' ?>
    </title>

    <!-- Bootstrap -->
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/css/bootstrap.min.css" rel="stylesheet">

    <link rel="stylesheet" href="/style.css">

</head>

<body class="bg-dark text-light" data-bs-theme="dark">

<div class="wrapper container py-4">

    <!-- HEADER -->
    <header class="mb-4">

        <div class="d-flex align-items-center gap-3 p-3 bg-body-tertiary rounded shadow-sm">

            <img
                src="/lab7-10/templates/logo.png"
                alt="logo"
                style="width: auto; height: 50px;"
            >

            <h1 class="h4 m-0">
                Мой блог
            </h1>

        </div>

    </header>

    <div class="row">

        <!-- MAIN -->
        <main class="col-lg-8">

            <div class="p-4 rounded shadow-sm bg-body-tertiary">

                <?= $content ?>

            </div>

        </main>

        <!-- SIDEBAR -->
        <aside class="col-lg-4">

            <div class="p-3 rounded shadow-sm bg-body-tertiary">

                <h3 class="h5 mb-3">Меню</h3>

                <ul class="list-group">

                    <li class="list-group-item bg-transparent">
                        <a class="text-decoration-none text-light" href="<?= url('') ?>">
                            Главная
                        </a>
                    </li>

                    <li class="list-group-item bg-transparent">
                        <a class="text-decoration-none text-light" href="<?= url('about') ?>">
                            Обо мне
                        </a>
                    </li>

                    <li class="list-group-item bg-transparent">
                        <a class="text-decoration-none text-light" href="<?= url('hello/User') ?>">
                            Hello
                        </a>
                    </li>

                    <li class="list-group-item bg-transparent">
                        <a class="text-decoration-none text-light" href="<?= url('bye/User') ?>">
                            Bye
                        </a>
                    </li>

                    <li class="list-group-item bg-transparent">
                        <a class="text-decoration-none text-light" href="<?= url('article/1') ?>">
                            Статья
                        </a>
                    </li>

                    <li class="list-group-item bg-transparent">
                        <a class="text-decoration-none text-light" href="<?= url('article/1/edit') ?>">
                            Редактирование
                        </a>
                    </li>

                </ul>

            </div>

        </aside>

    </div>

    <!-- FOOTER -->
    <footer class="text-center mt-4 text-secondary">
        Все права защищены
    </footer>

</div>

<script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/js/bootstrap.bundle.min.js"></script>

</body>
</html>